...
#define __MODULE_VERSION__ "0.11.0"
...
uint Node::node_cnt = 0; // ! Starting the node count from zero
Node* head = NULL;
...
Signal BlockSaving(BLOCKS_SAVE_SIGNAL_PIN, 0.20f, false, 2.0f);
...
void setup() 
{
    ...
    pinMode(BLOCKS_SAVE_SIGNAL_PIN, OUTPUT);
    // !SECTION
...
void loop() 
{
    ...
    if (BlockSaving.IsInReactionInterval(Logger.GetLastWriteTime()) && !BlockSaving.ChangeStateTimer.IsEnabled())
        BlockSaving.ChangeStateTimer.Enable();
    
    else if (!BlockSaving.IsInReactionInterval(Logger.GetLastWriteTime()) && BlockSaving.ChangeStateTimer.IsEnabled())
        BlockSaving.ChangeStateTimer.Disable();

     LeftTurning.TryBlink();
     RightTurning.TryBlink();
     BlockSaving.TryBlink();
}
